...
import { contextReturnVal } from "../utils/ContextApi";

const SearchResultHeader = () => {
  const [selectedMenu,setSelectedMenu] = useState('All');
  const {imageSearch,setImageSearch} = contextReturnVal()
  ...
  const clickHandler=(name)=>{
    let isTypImage = name === 'Images'
    setSelectedMenu(name);
    setImageSearch(isTypImage ? true : false)
  }
    ...
    <div className="p-[15px] pb-0 md:pr-5 md:pl-20 md:pt-7 border-b border-[#ebebeb] flex md:block flex-col items-center sticky top-0 bg-white">
      ...
      <div className="flex ml-[-12px] mt-3">
      {
        menu.map((menu,index)=>(
          ...
          onClick={()=>clickHandler(menu.name)}
          >
